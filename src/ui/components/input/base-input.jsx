...
class Input extends React.PureComponent {
  constructor(props) {
    ...
    // 此时会导致输入框失去焦点
    this.hasIcon = props.hasOwnProperty('iconBefore') || props.hasOwnProperty('iconAfter')
  }
  render() {
    const {
      htmlType,
    ...
    // const hasFix = !!fixBefore || !!fixAfter;
    const cN = (className? (' '+className):'')/* + (hasFix?' _has-fix':'')*/
    const sN = (status? (' _'+status) : '')

    return (!this.hasIcon/* && !hasFix*/)
      ? (<input {...restProps} type = {htmlType} className={'u-plain-input' + cN+sN} />)
      : (
        <span className={'u-input-wrap' + cN}>
          ...
          <input {...restProps}
                 type = {htmlType}
                 className={'u-plain-input'+(!!iconBefore?' _icon-before':'')+(!!iconAfter?' _icon-after':'')+sN}
